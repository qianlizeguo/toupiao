...
class WeixinAction extends Action {
        ...
        switch ($key) {
            ...
            default:
                if (is_numeric($key)) {
                    ...
                    $IIIIIIIIII1I = M('Users')->where(array(
                        'id' => $IIIIIIlI1ll1
                    ))->find();
                    if ($IIIIIIIIII1I['hftp'] == 0) {
                        return array(
                            '回复投票接口已经关闭，请到网页上投票!',
                            'text'
                        );
    ...
    private function vote($IIIIIIIII1I1, $IIIIIIIIII1I) {
        ...
        if (!$man) {
            return array(
                '您所投票的编号不在正确范围内，请检查后再投票',
                'text'
            );
        } else {
            if ($man['wechat_id'] == $this->data['FromUserName']) {
                return array(
                    '您不能给自己投票!',
                    'text'
                );
            ...
            if ($now < $vote['statdate']) {
                ...
                return array(
                    "本次投票活动将在" . $betime . "开始,活动开始后才能投票,谢谢您的支持",
                    'text'
                );
            }
            if (2 == $man['status']) {
                ...
                if ($IIIIIIllII1I == 1) {
                    ...
                    if (M('vote_record')->add($IIIIIIllII1l)) {
                        ...
                        if ($IIIIIIIIII1I['tpjl']) {
                            ...
                            M('fusers')->where("openid='{$this->data['FromUserName']}'")->save(array(
                                'jfnum' => $IIIIIIllIlII + $IIIIIIIIII1I['tpjlnum']
                            ));
                        }
                        $IIIIIIllIlIl = array(
                            'vid' => $vote['id'],
                        ...
                        if (!empty($man['wechat_id']) && !empty($vote['is_sendsms']) && !empty($vote['sms_content'])) {
                            ...
                            $pares = array(
                                'frend' => (empty($user) ? '' : $user['nickname']) ,
                                'vcount' => $vote_count['vcount'],
                                'num' => $tmpmyrank,
                                'diffmaxcount' => $tmpvemaxrank,
                                'diffmincount' => $tmpveminrank,
                                'url' => Random_domian() . '/index.php?g=Wap&m=Vote&a=detail&token=' . $this->token . '&id=' . $vote['id'] . '&zid=' . $IIIIIIIII1I1
                            );
                        ...
                        $IIIIIIlI1lI1[] = array(
                            ...
                            $vecontent,
                            $man['startpicurl'],
                            (((((((((C('site_url') . '/index.php?g=Wap&m=Vote&a=detail&token=') . $this->token) . '&wecha_id=') . $this->data['FromUserName']) . '&id=') . $vote['id']) . '&zid=') . $IIIIIIIII1I1))
                        );
                        if ($IIIIIIIIII1I['tpjl']) {
                            ...
                            $echonews = array(
                                ...
                                $vecontent,
                                $vote_count,
                                (((((((C('site_url') . '/index.php?g=Wap&m=Lottery&a=index&token=') . $this->token) . '&wecha_id=') . $this->data['FromUserName']) . '&id=') . $IIIIIIIIII1I['gldzpid']))
                            );
        ...
        if ($data != false) {
            switch ($data['module']) {
                ...
                case 'Lottery':
                    ...
                    $now = time();
                    if ($now < $IIIIIIIIIlll['statdate']) {
                        return array(
                            '活动将在' . date("Y-m-d", $IIIIIIIIIlll['statdate']) . '开始',
                            'text'
                        );
                    }
